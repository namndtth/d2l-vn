...
-->
Cùng xem xét tình huống mà giá trị đầu vào và đầu ra có cùng kích thước.
```{.python .input  n=2}
blk = Residual(3)
blk.initialize()
...
-->
Chúng ta cũng có một lựa chọn giảm kích thước chiều vào và rộng đi một nửa trong khi tăng số lượng kênh của đầu ra.
```{.python .input  n=3}
